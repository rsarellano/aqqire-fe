<template>
  <div class="flex flex-col gap-4 text-slate-600">
    <h1 class="pb-4 text-2xl font-semibold border-b border-slate-200">Login Information</h1>
...
v-tooltip="'Click to edit'" class="p-1 px-2 font-bold text-white bg-red-500 rounded cursor-pointer"
    ...
    <div class="flex flex-wrap gap-4 p-1">
      <div class="flex gap-2">
        <Checkbox v-model="model!.active" binary name="active" />
        <label for="active" class="capitalize">Active</label>
      </div>

      <div class="flex gap-2">
        <Checkbox v-model="model!.staff" binary name="staff" />
        <label for="staff" class="capitalize">Staff</label>
      </div>

      <div class="flex gap-2">
        <Checkbox v-model="model!.superuser" binary name="superuser" />
        <label for="superuser" class="capitalize">Superuser</label>
      </div>

      <div class="flex gap-2">
        <Checkbox v-model="model!.broker" binary name="broker" />
        <label for="broker" class="capitalize">Broker</label>
      </div>
    </div>
